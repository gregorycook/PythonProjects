...
name_indexed_voting_records = create_voting_dict()

def get_voting_vec(sen, voting_dict):
    return vec.Vec(set(range(len(voting_dict[sen]))), {k:v for (k,v) in enumerate(voting_dict[sen])})
...
def policy_compare(sen_a, sen_b, voting_dict):
    ...
    Example:
        ...
        -2
    """
    voting_a = get_voting_vec(sen_a, voting_dict)
    voting_b = get_voting_vec(sen_b, voting_dict)
